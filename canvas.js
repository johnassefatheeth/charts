...
class BarChart {
  ...
  draw() {
    const barColor = isValidColor(this.barColor)
      ? this.barColor
    ...
    scaledData.forEach((value, index) => {
      ...
      this.ctx.fillText(
        originalValue,
        x + barWidth / 2 - this.ctx.measureText(originalValue).width / 2,
        labelY
      );
      ...
      if (this.options.dataLabels) {
        ...
        const textX =
          x +
          barWidth / 2 -
          this.ctx.measureText(`Value ${index + 1}`).width / 2;
        const textWidth = this.ctx.measureText(`Value ${index + 1}`).width;
        const textHeight = parseInt(this.ctx.font, 10);
        const textOverlapsBar =
          value >= 0 &&
          textX < x + barWidth &&
          textX + textWidth > x &&
          dataLabelY > y &&
          dataLabelY < y + Math.abs(barHeight);
  ...
  draw() {
    ...
    this.data.forEach((value, index) => {
      if (!this.colors[index]) {
        this.colors[index] = `#${Math.floor(Math.random() * 16777215).toString(
          16
        )}`;
      }
      ...
      this.ctx.fillStyle =
        index === this.hoverIndex ? "red" : this.colors[index];
      this.ctx.beginPath();
      ...
      const labelX =
        this.canvas.width / 2 +
        (this.canvas.height / 4) * Math.cos(middleAngle);
      const labelY =
        this.canvas.height / 2 +
        (this.canvas.height / 4) * Math.sin(middleAngle);
...
class LineChart {
  ...
  draw() {
    ...
    this.ctx.moveTo(
      ...
      chartHeight -
        padding -
        ((this.data[0] - minValue) / valueRange) * (chartHeight - 2 * padding)
    );
    for (let i = 1; i < this.data.length; i++) {
      ...
      const y =
        chartHeight -
        padding -
        ((this.data[i] - minValue) / valueRange) * (chartHeight - 2 * padding);
      this.ctx.lineTo(x, y);
    }
...
function handleHover(event, chart) {
  ...
  switch (chart) {
    case barChart:
      const barWidth =
        parseInt(document.getElementById("barWidth").value, 10) || 40;
      const barSpacing = 20;
    ...
    case pieChart:
      ...
      chart.data.forEach((value, index) => {
        ...
        // Check if the mouse is within the current slice
        if (
          distance <= chart.canvas.height / 2 &&
          angle >= startAngle &&
          angle <= endAngle
        ) {
    ...
    case lineChart:
      const padding = 10;
      const xStep =
        (chart.canvas.width - 2 * padding) / (chart.data.length - 1);
...
barCanvas.addEventListener("mousemove", (event) =>
  handleHover(event, barChart)
);
pieCanvas.addEventListener("mousemove", (event) =>
  handleHover(event, pieChart)
);
lineCanvas.addEventListener("mousemove", (event) =>
  handleHover(event, lineChart)
);
barCanvas.addEventListener("mouseout", () => {
  tooltip.style.display = "none";
});
pieCanvas.addEventListener("mouseout", () => {
  tooltip.style.display = "none";
});
lineCanvas.addEventListener("mouseout", () => {
  tooltip.style.display = "none";
});
...
pieCanvas.addEventListener("mousemove", (event) =>
  pieChart.handleMouseMove(event)
);
pieCanvas.addEventListener("mouseout", () => pieChart.handleMouseOut());
lineCanvas.addEventListener("mousemove", (event) =>
  lineChart.handleMouseMove(event)
);
